...
const ProductDetailsPopOver: React.FC<ProductDetailsPopOverProps> = ({ products, trigger, triggerText }) => {
            ...
            <PopoverTrigger asChild>
                {trigger ? (
                    ...
                    <button className="font-medium focus:outline-none">
                        {triggerText || (products.length === 1
                            ? `${products[0].product || products[0].name} x ${products[0].quantity}`
                            : `${products[0].product || products[0].name} x ${products[0].quantity} +${products.length - 1} more`)}
                    </button>
                )}
            </PopoverTrigger>
