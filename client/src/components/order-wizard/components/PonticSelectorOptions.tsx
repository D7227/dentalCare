...
const PonticSelectorOptions = ({
  ...
  if (readonly) {
    ...
    const selectedOption = ponticOptions.find(
      (option) => option.name === selected
    );
    if (!selectedOption)
      return <div className="text-gray-500 italic">No pontic selected</div>;
    return (
      ...
      {ponticOptions.map((option) => (
        ...
        >
          <button
            ...
            className={`border-2 rounded-lg transition-all p-3 duration-150 ${
              selected === option.name
                ? "border-[#11AB93] bg-[#E6FAF7]"
...
export default PonticSelectorOptions;
