...
const OrderTypeSection = ({ formData, setFormData, render = false, editing = false }: OrderTypeSectionProps) => {
  ...
  const renderInputs = (data: any, onValueChange: any) => (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {/* Digital Card */}
      <Card
        className={`cursor-pointer transition-all duration-200 hover:shadow-md border-2 ${data.orderMethod === "digital"
      ...
      >
        <CardContent className="p-4 text-center">
          <div
            className={`w-12 h-12 mx-auto mb-3 rounded-full flex items-center justify-center ${data.orderMethod === "digital"
              ? "bg-teal-100"
              : "bg-gray-100"
              }`}
          >
            <Smartphone
              className={`w-6 h-6 ${data.orderMethod === "digital"
                ? "text-teal-600"
                : "text-gray-600"
                }`}
            />
          ...
          <h3
            className={`font-medium text-sm ${data.orderMethod === "digital"
              ? "text-teal-700"
              : "text-gray-700"
              }`}
          >
      ...
      {/* Manual Card */}
      <Card
        className={`cursor-pointer transition-all duration-200 hover:shadow-md border-2 ${data.orderMethod === "manual"
          ? "border-teal-500 bg-teal-50 shadow-md"
          : "border-gray-200 hover:border-gray-300"
          }`}
      ...
      >
        <CardContent className="p-4 text-center">
          <div
            className={`w-12 h-12 mx-auto mb-3 rounded-full flex items-center justify-center ${data.orderMethod === "manual"
              ? "bg-teal-100"
              : "bg-gray-100"
              }`}
          >
            <FileText
              className={`w-6 h-6 ${data.orderMethod === "manual"
                ? "text-teal-600"
                : "text-gray-600"
                }`}
            />
          ...
          <h3
            className={`font-medium text-sm ${data.orderMethod === "manual"
              ? "text-teal-700"
              : "text-gray-700"
              }`}
          >
    ...
    <>
      <Card>
        <CardHeader className="py-3 flex flex-row items-center justify-between">
          <CardTitle className="text-xl font-semibold">Order Type</CardTitle>
          {editing && (
            <button
              onClick={() => {
      ...
      >
        <div className="space-y-4 p-2">{renderInputs(editData, handleEditValue)}</div>
